...
// 💳 Nequi + WhatsApp (ENV)
const NEQUI_PHONE = (process.env.NEQUI_PHONE || "").trim();
const NEQUI_NAME = (process.env.NEQUI_NAME || "").trim();
const NEQUI_NOTE = (process.env.NEQUI_NOTE || "").trim();
const WHATSAPP_NUMBER = (process.env.WHATSAPP_NUMBER || "3504438874").replace(/\D/g, "");
...
const WOMPI_EVENTS_SECRET = (process.env.WOMPI_EVENTS_SECRET || "").trim(); // opcional webhook
...
function publicBaseUrl(req) {
  // importante en Railway: confiar en x-forwarded-*
...
function ensureOrdersForRifa(ordersDB, rifaId) {
  ...
  if (!ordersDB.byRifa[rifaId]) ordersDB.byRifa[rifaId] = { orders: {} };
...
function validateBalotoNums(nums, min = 1, max = DEFAULT_N) {
  ...
  if (nums.some((x) => !Number.isInteger(x) || x < min || x > max)) return false;
...
async function fetchBalotoReal() {
  ...
  // Recolectar dígitos visibles (heurística)
  const digits = [];
  $("*").each((_, el) => {
  ...
  throw new Error("No fue posible extraer resultados reales desde Baloto (estructura cambió).");
}

// =========================
// Semilla y sello
// =========================
function buildSeed({ rifaId, org, balotoNums, superBalota, k, totalBoletas, modeLabel }) {
...
async function computeSorteo({ rifa, rifaId, balotoParam, kParam }) {
  ...
  if (!validateK(k)) return { ok: false, status: 400, error: "k inválido (1..5)" };

  const wantsReal = safeStr(balotoParam).toLowerCase() === "real";
  if (wantsReal && isLockedAfterOfficial(rifa)) {
    return { ok: false, status: 400, error: "Sorteo oficial ya realizado. Rifa bloqueada." };
  }
  ...
  if (mustDisallowSimulation(rifa, balotoParam)) {
    return {
      ok: false,
      status: 400,
      error: "Simulación deshabilitada. Solo se permite sorteo oficial (baloto=real).",
    };
  }
...
// =========================
function computeOfficialManual({ rifa, rifaId, balotoStr, superBalotaStr, kParam, nota }) {
  const k = kParam != null ? Number(kParam) : Number(rifa.k);
  if (!validateK(k)) return { ok: false, status: 400, error: "k inválido (1..5)" };

  const parsed = parseBalotoParam(balotoStr);
  if (!parsed || !validateBalotoNums(parsed.nums)) {
    return { ok: false, status: 400, error: "Baloto manual inválido. Usa: 03-11-19-27-41" };
  }
...
function ensureRifaSalesFields(db, rifaId) {
  const rifa = db.rifas[rifaId];
  if (rifa.totalBoletas == null) rifa.totalBoletas = nCk(DEFAULT_N, Number(rifa.k));
...
function approveOrderAssignTickets({ db, rifaId, order }) {
  ...
  if (!Number.isFinite(qty) || qty < 1) return { ok: false, status: 400, error: "Cantidad inválida" };
  ...
  if (end > total) {
    return { ok: false, status: 400, error: "No hay suficientes boletas disponibles" };
  }
  ...
  if (Number(rifa.soldCount) >= Number(rifa.totalBoletas)) rifa.estado = "CERRADA";
...
<div style="margin-top:10px;padding:16px;border-radius:14px;background:#f0f9ff;border:1px solid #cfe8ff;">
  ...
  <div style="margin-bottom:6px;"><b>Precio por boleta:</b> $${toCOP(rifa.precio)} COP</div>
  <div style="margin-bottom:6px;"><b>Disponibles:</b> ${toCOP(disponibles)} de ${toCOP(totalBoletas)}</div>
  ...
  let pagoFooter = "";
  if (!pagoOk) {
    pagoFooter = `<div style="margin-top:16px;opacity:.7;font-size:13px;">
      (Pago Nequi no configurado: agrega NEQUI_PHONE y NEQUI_NAME)
    </div>`;
  } else if (!abierta) {
    pagoFooter = `<div style="margin-top:16px;opacity:.75;font-size:13px;">
      (La rifa no está abierta para ventas ahora. Estado: <b>${rifaEstado(rifa)}</b>)
    </div>`;
  } else {
...
<body style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial; background:#f5f7fb; margin:0; padding:20px;">
  <div style="max-width:720px;margin:0 auto;background:#fff;border-radius:16px;box-shadow:0 8px 30px rgba(0,0,0,.08); padding:22px;">
    ...
    <div style="opacity:.85;">RifaID: <b>${rifaId}</b> — Estado: <b>${rifaEstado(rifa)}</b></div>
    <hr style="margin:16px 0;border:none;border-top:1px solid #e8edf5;"/>

    <div style="margin-bottom:10px;"><b>Precio por boleta:</b> $${toCOP(rifa.precio)} COP</div>
    <div style="margin-bottom:10px;"><b>Disponibles:</b> ${toCOP(disponibles)} de ${toCOP(totalBoletas)}</div>
...
function renderOrdenPage({ rifaId, rifa, order }) {
  ...
  const wa = order.waLink || waLink({ rifaId, orderId: order.orderId, totalPagar: order.totalPagar });
  ...
  const wompiNote = order.wompiTransactionId
...
<body style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial; background:#f5f7fb; margin:0; padding:20px;">
  <div style="max-width:720px;margin:0 auto;background:#fff;border-radius:16px;box-shadow:0 8px 30px rgba(0,0,0,.08); padding:22px;">
    ...
    <div style="opacity:.85;">RifaID: <b>${rifaId}</b> — Estado rifa: <b>${rifaEstado(rifa)}</b></div>
    <hr style="margin:16px 0;border:none;border-top:1px solid #e8edf5;"/>
...
// PDF Acta OFICIAL
// =========================
async function generateActaPDF({ req, res, rifa, rifaId, officialResult, actaConsecutivo }) {
  ...
  res.setHeader("Content-Disposition", `inline; filename="${actaConsecutivo}.pdf"`);

  const doc = new PDFDocument({ size: "A4", margin: 50 });
  doc.pipe(res);

  doc.fontSize(18).font("Helvetica-Bold").text("ACTA OFICIAL DE SORTEO", { align: "center" });
  doc.moveDown(0.5);

  doc.roundedRect(50, doc.y, 495, 40, 10).fillAndStroke("#ecfff1", "#1f8f3a");
  doc.fillColor("#0b5b1f").fontSize(12).font("Helvetica-Bold").text(sello, 60, doc.y - 28);
  doc.fillColor("#000");
  doc.moveDown(1.5);
  ...
  if (officialResult.baloto.nota) doc.text(`Nota: ${officialResult.baloto.nota}`);
  doc.text(`k: ${officialResult.k}`);
  doc.text(`Total de boletas (C(43,k)): ${officialResult.totalBoletas}`);
  doc.font("Helvetica-Bold").text(`Boleta ganadora: ${officialResult.boletaGanadoraFmt}`);
  ...
  if (officialResult.baloto.fuenteReal) doc.text(`Fuente Baloto real: ${officialResult.baloto.fuenteReal}`);
  doc.text(`JSON verificable: ${verifyUrl}`);
  doc.text(`Estado / sello / bloqueo: ${estadoUrl}`);

  doc.image(qrImage, 400, 190, { width: 150, height: 150 });
  doc.font("Helvetica").fontSize(10).fillColor("#333").text("Escanea para verificar el JSON", 395, 345, {
    width: 170,
    align: "center",
  });
...
app.post("/api/rifas", (req, res) => {
  try {
    ...
    if (!org) return res.status(400).json({ ok: false, error: "Falta org (cédula o NIT)" });
    if (!validateK(k)) return res.status(400).json({ ok: false, error: "k inválido (1..5)" });
    if (!Number.isFinite(precio) || precio < 1) return res.status(400).json({ ok: false, error: "precio inválido" });
    ...
    res.status(500).json({ ok: false, error: e?.message || "Error creando rifa" });
  }
});

// =========================
// ✅ Crear rifa DEMO (RUTA QUE TE FALTABA)
// =========================
app.get("/crear-demo", (req, res) => {
  try {
    ...
    writeRifasDB(db);
    res.json({ ok: true, rifaId, totalBoletas, links: { sorteo: `/rifas/${rifaId}/sorteo?baloto=demo&k=2`, comprar: `/rifas/${rifaId}/comprar` } });
  } catch (e) {
...
app.get("/rifas/:rifaId/comprar", (req, res) => {
  ...
  if (!NEQUI_PHONE || !NEQUI_NAME) return res.status(400).send("Pago Nequi no configurado.");
  if (!isRifaAbierta(rifa)) return res.status(400).send(`Rifa no abierta para ventas. Estado: ${rifaEstado(rifa)}`);
...
app.post("/rifas/:rifaId/ordenes", (req, res) => {
  try {
    ...
    if (!NEQUI_PHONE || !NEQUI_NAME) return res.status(400).json({ ok: false, error: "Pago Nequi no configurado" });
    if (!isRifaAbierta(rifa)) return res.status(400).json({ ok: false, error: `Rifa no abierta. Estado: ${rifaEstado(rifa)}` });
    ...
    if (!Number.isFinite(cantidad) || cantidad < 1) return res.status(400).json({ ok: false, error: "Cantidad inválida" });
    ...
    const disponibles = Math.max(0, totalBoletas - sold);
    if (cantidad > disponibles) return res.status(400).json({ ok: false, error: "No hay suficientes boletas disponibles" });
    ...
    res.status(500).json({ ok: false, error: e?.message || "Error creando orden" });
  }
});

// =========================
// Ver orden + Opción B (auto-confirmación sin webhook)
// =========================
app.get("/rifas/:rifaId/orden/:orderId", async (req, res) => {
  ...
  // OPCIÓN B: al regresar del widget (?wompi=1)
  const wantsCheck = String(req.query.wompi || "") === "1";
  if (wantsCheck && safeStr(order.estado).toUpperCase() === "PENDIENTE") {
    try {
      if (WOMPI_PRIVATE_KEY) {
        // buscar transacción por reference (orderId)
        const txUrl = `https://production.wompi.co/v1/transactions?reference=${encodeURIComponent(orderId)}`;
...
// =========================
// ✅ Pagar con Wompi (Widget) por orden — CORREGIDO
// =========================
app.get("/rifas/:rifaId/orden/:orderId/pagar", (req, res) => {
  ...
  const signature = wompiIntegritySignature({ reference, amountInCents: String(amountInCents), currency });
  const base = publicBaseUrl(req);

  // Opción B: volver a la orden con wompi=1
...
<body style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;background:#f5f7fb;margin:0;padding:20px;">
  <div style="max-width:720px;margin:0 auto;background:#fff;border-radius:16px;box-shadow:0 8px 30px rgba(0,0,0,.08);padding:22px;">
    ...
    <div style="opacity:.85;">Orden: <b>${orderId}</b> — Total: <b>$${toCOP(order.totalPagar)} COP</b></div>
...
app.post("/webhooks/wompi", express.json({ type: "*/*" }), (req, res) => {
  try {
    if (!WOMPI_EVENTS_SECRET) return res.status(400).send("WOMPI_EVENTS_SECRET no configurado.");
    ...
    if (!reference || !reference.startsWith("ord_")) return res.status(200).json({ ok: true, ignored: true });
    ...
    if (!found || !foundRifaId) return res.status(200).json({ ok: true, notFound: true });
    ...
    if (status === "APPROVED") {
      const db = rifasDB;
      const r = approveOrderAssignTickets({ db, rifaId: foundRifaId, order: found });
      if (!r.ok) {
        ordersDB.byRifa[foundRifaId].orders[reference] = found;
        writeOrdersDB(ordersDB);
...
<body style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;background:#f5f7fb;margin:0;padding:20px;">
  <div style="max-width:1100px;margin:0 auto;background:#fff;border-radius:16px;box-shadow:0 8px 30px rgba(0,0,0,.08);padding:20px;">
    ...
    <div style="opacity:.8;margin-top:6px;">RifaID: <b>${rifaId}</b> — Estado rifa: <b>${rifaEstado(rifa)}</b></div>
...
app.post("/rifas/:rifaId/admin/ordenes/:orderId/aprobar", (req, res) => {
  try {
    ...
    if (safeStr(order.estado).toUpperCase() !== "PENDIENTE") {
      return res.status(400).json({ ok: false, error: "Orden no está en PENDIENTE" });
    }
    ...
    res.status(500).json({ ok: false, error: e?.message || "Error aprobando orden" });
  }
});
...
app.post("/rifas/:rifaId/admin/ordenes/:orderId/rechazar", (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, error: e?.message || "Error rechazando orden" });
  }
});

// =========================
// Admin: aprobar por LINK (GET) + redirect
// =========================
app.get("/rifas/:rifaId/admin/ordenes/:orderId/aprobar-web", (req, res) => {
  try {
    ...
    if (safeStr(order.estado).toUpperCase() !== "PENDIENTE") return res.status(400).send("Orden no está en PENDIENTE");
...
// =========================
// Admin: rechazar por LINK (GET) + redirect
// =========================
...
app.post("/rifas/:rifaId/admin/estado", (req, res) => {
  try {
    ...
    if (!["ABIERTA", "CERRADA", "FINALIZADA"].includes(estado)) {
      return res.status(400).json({ ok: false, error: "Estado inválido (ABIERTA|CERRADA|FINALIZADA)" });
    }

    const db = readRifasDB();
    const rifa = db.rifas?.[rifaId];
    if (!rifa) return res.status(404).json({ ok: false, error: "Rifa no encontrada" });
    ...
    res.status(500).json({ ok: false, error: e?.message || "Error cambiando estado" });
  }
});

// =========================
// Acta PDF (solo OFICIAL) — sella y bloquea
// =========================
app.get("/rifas/:rifaId/acta", async (req, res) => {
  try {
    ...
    if (safeStr(balotoParam).toLowerCase() !== "real") {
      return res.status(400).send("Acta solo se genera para sorteo oficial (baloto=real).");
    }

    // ya bloqueada: reimprime
    if (isLockedAfterOfficial(rifa) && rifa.officialResult && rifa.actaConsecutivo) {
    ...
    // sorteo oficial automático
    const officialResult = await computeSorteo({ rifa, rifaId, balotoParam: "real", kParam });
    if (!officialResult.ok) return res.status(officialResult.status || 400).send(officialResult.error);
...
app.post("/rifas/:rifaId/oficial-manual", (req, res) => {
  try {
    ...
    if (isLockedAfterOfficial(rifa)) {
      return res.status(400).json({ ok: false, error: "Rifa ya bloqueada (ya tiene sorteo oficial)." });
    }
    ...
    if (!officialResult.ok) {
      return res.status(officialResult.status || 400).json({ ok: false, error: officialResult.error });
    }
    ...
    res.status(500).json({ ok: false, error: e?.message || "Error en oficial-manual" });
  }
});
